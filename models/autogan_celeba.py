...
class AutoGAN_CelebA(object):
  def __init__(self, sess, input_height=128, input_width=128, is_crop=True,
  ...
  def train(self, config):
    """Train DCGAN"""
    data = glob(os.path.join(config.stash_dir, 'celebA', self.input_fname_pattern))
    ...
    for epoch in range(config.epoch):
      data = glob(os.path.join(
        config.stash_dir, 'celebA', self.input_fname_pattern))
